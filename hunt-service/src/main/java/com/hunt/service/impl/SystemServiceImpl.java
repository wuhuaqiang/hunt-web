...
public class SystemServiceImpl implements SystemService {
    ...
    @Override
    public void clearAuthorizationInfoCacheByUserId(Integer userId) {
    ...
    /*    log.debug("clear authorization info cache by roleId: {}", roleId);
        ...
        if (list.size() > 0) {
            for (Integer id : list) {
                List<Integer> userIds = sysUserRoleOrganizationMapper.selectByRoleOrganizationId(id);
                if (userIds.size() > 0) {
    ...
    @Override
    public Integer insertSysDataGroup(SysDataGroup sysDataGroup) {
    ...
    @Override
    public Integer insertSysDataItem(SysDataItem sysDataItem) {
        sysDataItemMapper.insert(sysDataItem);
        return sysDataItem.getId();
    }

    @Override
    public boolean isExistDataItemKeyName(String key, Integer groupId) {
        return sysDataItemMapper.isExistName(key, groupId);
    }

    @Override
    public void deleteDataItemById(Integer id) {
    ...
    @Override
    public boolean isExistDataItemNameExcludeId(Integer id, String key, Integer groupId) {
    ...
    @Override
    public SysDataItem selectDataItemById(Integer id) {
        return sysDataItemMapper.selectById(id);
    }

    @Override
    public String selectDataItemByKey(String key, Integer groupId) {
    ...
    @Override
    public Integer insertIp(SysIpForbidden sysIpForbidden) {
    ...
    @Override
    public boolean isExistIpExcludeId(String ip, Integer id) {
    ...
    public boolean selectIPForbiddenStatus() {
        SysDataItem sysDataItem = sysDataItemMapper.selectById(4);
        if (sysDataItem != null && sysDataItem.getKeyValue().equals("true")) {
            return true;
        }
        return false;
    }
